...
class register():
 ...
 def store_coordinate(self):
  self.name_sp=[
 '门口_position','门口_orientation','盆栽_position','盆栽_orientation',]
# '充电_position','充电_orientation','孟孟_position','孟孟_orientation',
# '刘森_position','刘森_orientation','徐志浩_position','徐志浩_orientation',
...
# '44_orientation','44_position','45_orientation','45_position','46_orientation','46_position',]
 ...
 def store_update(self,name,dictionary):
  if name not in dictionary:
   rospy.loginfo('请输入%s的位置'%name.split('_')[0])
   data=rospy.wait_for_message('/initialpose', PoseWithCovarianceStamped)
   if name.split('_position')>1:
    dictionary['%s'%name]=data.pose.pose.position
    dictionary['%s'%name.split('_position')[0]+'_orientation']=data.pose.pose.orientation
   if name.split('_orientation')>1:
    dictionary['%s'%name]=data.pose.pose.orientation
    dictionary['%s'%name.split('_orientation')[0]+'_position']=data.pose.pose.position
 ...
 def office_desk_matrix_position(self,column,row):
  self.defination()
  point=self.office_matrix_orientation['%s%s'%(row,column)]
  return point


 def office_desk_matrix_orientation(self,column,row):
  self.defination()
  orientation=self.office_matrix_orientation['%s%s'%(row,column)]
  return orientation
 ...
 '孟孟':pose['孟孟_position'],'刘森':pose['刘森_position'],'徐志浩':pose['徐志浩_position']
}

  self.direct_orientation_dic={
 '门口':pose['门口_orientation'],'盆栽':pose['盆栽_orientation'],'充电':pose['充电_orientation'],
 '孟孟':pose['孟孟_orientation'],'刘森':pose['刘森_orientation'],'徐志浩':pose['徐志浩_orientation']
}
