...
class excel_to_coordinate():
 def __init__(self):
  ...
  while not rospy.is_shutdown():
   ...
   print  '\n\n','centremap[1]',centremap[1],'centremap[0]',centremap[0]
   height=sheet.get_highest_row()
   print 'tableheight',height
   height+=centremap[1]
   print 'mapcentre :',(2-centremap[0]),height
   ...
   print 'robotpose',robotpose,'odom',[Pose_x,Pose_y]
   robot_height=height-robotpose[1]
   print 'robotpose height',robot_height,'robotpose',(centremap[0]+robotpose[0])
   robot_column = '%s%s'%(get_column_letter((2-centremap[0])+robotpose[0]),robot_height)
   ...
   print 'pointpose',pointpose
   pointpose_height=height-pointpose[1]
   pointed_column = '%s%s'%(get_column_letter((2-centremap[0])+pointpose[0]),pointpose_height)
