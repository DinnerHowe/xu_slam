...
############################ main ###########################3
 def WarningPub(self,right_weight,middle_weight,left_weight):
  ...
  #AnalyseResult=self.ObstacleAnalyse(self.right_weight,self.middle_weight,self.left_weight)
...
################# laser callback#######################
 def scan_callback(self, scan):
  ...
  """######################## AUTO Avoidance ##########################
  AvoidAction=AbstacleAvoidance()
  AvoidAction.AvoidanceAction(self.warninfo)"""
...
class AbstacleAvoidance():
 ...
 def AvoidanceAction(self, info):
  ...
  if info=='RightObstacle':
   self.Actionmodel(linearsp, act_speed, info)

  elif info=='MiddleObstacle':
   self.Actionmodel(linearsp, act_speed, info)

  elif info=='LeftObstacle':
   self.Actionmodel(linearsp, 0-act_speed, info)

  elif info=='LeftDanger':
   self.Actionmodel(linearsp-0.1, 0-act_speed, info)

  elif info=='RightDanger':
   self.Actionmodel(linearsp-0.1, act_speed, info)

  elif info=='AllClear':
   self.Actionmodel(linearsp-0.1, 0, info)
...
class SlamActionSol4():
 print "slam action solution 4"
#################### normalize angel ##########################
...
######################## initial setting#######################
  ...
  rospy.init_node('slam_base_action4', anonymous=False)
