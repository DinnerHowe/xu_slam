...
#################### Turning Angle ##########################
 def TurnAngle(self, goal_angle,turn_speed):
  ...
  while abs(goal_angle)-abs(turn_angle)-abs(turn_speed) > 0 and not rospy.is_shutdown():
   # Publish the Twist message and sleep 1 cycle         
   self.cmd_vel.publish(move_cmd)
   print "move cmd:%s"%move_cmd         
   # Get the current rotation
   (position, rotation) = self.get_odom()           
...
######################## initial setting#######################
  ...
  LoopNum=int(travel_distance/5.0)
################### running loop ######################            
...
####################### turning #######################            
   ...
   sub_goal_angle = pi*2
   AngularSpeed=sub_goal_angle/(sub_goal_angle)
   #turn to sub goal0           
