...
#include "facerobot/voiceRecog.h"
#include "voice_msg/recog_result.h"
#include "facerobot/qisr.h"
#include "facerobot/msp_cmn.h"
#include "facerobot/msp_errors.h"
#include "facerobot/msp_types.h"
...
const char * GRM_BUILD_PATH      = "fo|res/asr/GrmBuilld";//"res/asr/GrmBuilld"; //构建离线语法识别网络生成数据保存路径
const char * GRM_FILE            = "/home/turtlebot/xu_slam/bin/command.bnf"; //构建离线识别语法网络所用的语法文件

char * filename="/home/turtlebot/xu_slam/bin/wav/recordfile.wav";
const char *rec_rslt = NULL;
bool flag1=false;
bool flag2=false;

map<string,int> semantic;   
...
int run_asr(UserData *udata)
{
	...
	if (NULL != rec_rslt)
	       ...
	       printf("%s\n", rec_rslt);
               ...
               ofstream outfile;
               outfile.open("/home/turtlebot/xu_slam/src/facerobot/src/result.xml");
               if(outfile.is_open())
               {
                outfile<<rec_rslt<<endl;
               }
               else
               {
                cout<<"不能打开文件！"<<endl;
...
int  main(int argc, char **argv)
{
  ...
  ros::NodeHandle n;
  ros::Publisher result_pub = n.advertise<voice_msg::recog_result>("recog_result", 1000);
  ros::Rate loop_rate(10);     
  voice_msg::recog_result content;
  if(pre_run_asr() && (NULL != rec_rslt))//调用语音识别
  {
