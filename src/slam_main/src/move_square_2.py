...
#move
 def move(self,waypoints,i):
  ...
  if not finished_within_time:
   ...
   rospy.loginfo("Timed out achieving goal,setting closer goal")

   waypoints,i=self.goal_spliter(goal,waypoints,i)
  ...
  #if finished_within_time and not state == GoalStatus.SUCCEEDED:
...
################################################################################
#goal_spliter
 def goal_spliter(self,goal,waypoints,i):

  rospy.loginfo('cancel goal and generate a new goal')
  self.move_base.cancel_goal()

  origin,new = MoveBaseGoal(), MoveBaseGoal()
  ...
  if i>0:
   origin.target_pose.pose = waypoints[i-1]
  if i==0:
   origin.target_pose.pose = waypoints[3]
  try:
  ...
  if goal.target_pose.pose.position.x == origin.target_pose.pose.position.x:
   if origin.target_pose.pose.position.y != goal.target_pose.pose.position.y:
    ...
    new.target_pose.pose.position.x = goal.target_pose.pose.position.x
   else:
    rospy.loginfo('target and current pose is in same position')
    pass

  #y==y
  if goal.target_pose.pose.position.y == origin.target_pose.pose.position.y:
   if origin.target_pose.pose.position.x != goal.target_pose.pose.position.x:
    ...
    new.target_pose.pose.position.y = goal.target_pose.pose.position.y
   else:
    rospy.loginfo('target and current pose is in same position')
    pass
  ...
  while not state==GoalStatus.SUCCEEDED: #state==GoalStatus.ABORTED or state==GoalStatus.REJECTED:
   goal,state = self.goal_compensator(goal,origin)
   #print state,goal
...
##################################################################################
#goal_compensator
 def goal_compensator(self,goal,origin):
